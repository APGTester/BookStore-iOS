...
public struct BookStore {
    ...
    @MainActor public static let defaultSearchEngine = SearchEngine()
    ...
    @MainActor public static func fetchNewReleases(completionHandler: @escaping (Result<NewBooksResponse, Error>) -> Void) {
    ...
    /// - Parameter completionHandler: The completion handler to call when the fetch request is complete. This handler is executed on the main queue.
    @MainActor public static func fetchInfo(with isbn13: String, completionHandler: @escaping (Result<BookInfo, Error>) -> Void) {
