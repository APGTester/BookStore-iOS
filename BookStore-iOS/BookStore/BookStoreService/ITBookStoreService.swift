...
struct ITBookStoreService: BookStoreService {
    ...
    @MainActor func fetchNewReleases(completionHandler: @escaping (Result<NewBooksResponse, Error>) -> Void) {
        BookStore.fetchNewReleases(completionHandler: completionHandler)
    }
    
    @MainActor func fetchInfo(with isbn13: String, completionHandler: @escaping (Result<BookInfo, Error>) -> Void) {
